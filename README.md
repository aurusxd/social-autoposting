...
`caddy reload` работает через admin API. Если в глобальном блоке Caddyfile
стоит `admin off`, команда не сработает и правку придётся применять
перезапуском контейнера — это секунда-другая недоступности всех сайтов за этим
прокси, так что время стоит согласовать с их владельцем:

```bash
docker restart ИМЯ-КОНТЕЙНЕРА-ПРОКСИ
```
